...
function tlistTop() {
	...
	for (var i = 0; i <= 4; i++) {
		thisHeight =
			document.getElementsByClassName('sethome-con')[i].offsetTop +
			divsTop -
			80
...
function if_btn() {
	var btn_obj =
		document.getElementById('kw') || document.getElementById('kw-2')
...
$('.list-link-4').hover(
	function () {
		...
		if (_thisTit != '') {
			//判断条件，当前元素的data-title不等于空才执行下面的代码
...
$(document).keydown(function (e) {
	if (e.keyCode == 13 && oUl.style.display == 'block') {
		...
		oUl.style.display = 'none'
		//alert('你按下了Enter');
	} else if (e && e.keyCode == 40 && oUl.style.display == 'block') {
		//下
		//active
	...
	} else if (e && e.keyCode == 38 && oUl.style.display == 'block') {
		// 上
